        ...
        <div class="x_panel">
            ...
            @if(!empty($order->portfolio->name))
                <div class="card col-md-6"> <a href="{{strtolower ( $order->portfolio->category_id)}}/{{ $order->portfolio->slug  }}">                        <img class="card-img-top" style="max-width: 220px; max-height: 220px" src="{{ $order->portfolio->photo->file }}" alt="Card image cap">
                        <a href="{{ route('order.view', ['order' => $order->id]) }}">
                            <br>
                            <button class="btn-lg btn-success">Print</button>
                        </a>
                    <ul class="list-group">
                        <li class="list-group-item">Id: <strong>{{ $order->id }} </strong></li>
                        <li class="list-group-item">Name: {{ $order->portfolio->name }} <strong>{{ $order->price }} </strong></li>
